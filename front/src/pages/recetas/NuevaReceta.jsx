...
export default function NuevReceta() {
  ...
  const [tiempoCocinado, setTiempoCocinado] = useState("");
  const [porciones, setPorciones] = useState("");
  const [caloriasPorPorcion, setCaloriasPorPorcion] = useState("");
  const [tiposComidaDisponibles, setTiposComidaDisponibles] = useState([]);
  const [tiposComidaSeleccionados, setTiposComidaSeleccionados] = useState([]);
  ...
  useEffect(() => {
    if (!ingredientes || !Array.isArray(ingredientes)) {
      console.error("Error: Los ingredientes no están disponibles o no son un array válido");
    }
  }, [ingredientes]);
  ...
  const handleGuardar = async () => {
    ...
    try {
      ...
      const nuevaReceta = {
        ...
        caloriasPorPorcion: parseInt(caloriasPorPorcion, 10),
  ...
  const handleChangePorciones = (e) => {
    const valor = e.target.value.replace(/[^0-9]/g, '');
    setPorciones(valor);
  };

  const handleChangeCaloriasPorPorcion = (e) => {
    const valor = e.target.value.replace(/[^0-9]/g, '');
    setCaloriasPorPorcion(valor);
  };
